...
sg_key = os.getenv("SENDGRID_API_KEY", "NOT FOUND")
st.sidebar.markdown(f"**SG Key:** `{sg_key[:15]}...`" if sg_key != "NOT FOUND" else "**SG Key:** NOT FOUND")
...
html, body, [class*="css"] { font-family: 'JetBrains Mono', monospace; background-color: #0c0e14; color: #e2e8f0; }
.main { background-color: #0c0e14; }
h1, h2, h3 { font-family: 'Syne', sans-serif; }
.stButton > button { background: linear-gradient(135deg, #3b82f6, #1d4ed8); color: white; border: none; border-radius: 8px; padding: 12px 32px; font-family: 'JetBrains Mono', monospace; font-size: 14px; font-weight: 600; letter-spacing: 1px; width: 100%; transition: all 0.2s; }
.stButton > button:hover { background: linear-gradient(135deg, #2563eb, #1e40af); transform: translateY(-1px); }
.cluster-card { background: #12151e; border: 1px solid #1e2333; border-left: 3px solid #3b82f6; border-radius: 10px; padding: 20px 24px; margin: 12px 0; }
.flag-item { background: #1a0b0e; border: 1px solid #3a1520; border-radius: 6px; padding: 8px 12px; margin: 4px 0; font-size: 12px; color: #fca5a5; }
.badge { display: inline-block; padding: 2px 10px; border-radius: 20px; font-size: 11px; font-weight: 600; letter-spacing: 1px; }
...
<div style='padding:32px 0 24px'>
  <div style='font-size:10px;letter-spacing:4px;text-transform:uppercase;color:#3b82f6;margin-bottom:8px'>Multi-Agent Pipeline</div>
  <h1 style='font-family:Syne,sans-serif;font-size:36px;font-weight:800;color:#f0f4ff;margin:0 0 8px'>KB Article Generator</h1>
  <p style='color:#475569;font-size:13px;margin:0'>Upload closed tickets → clusters detected → articles drafted → emailed to Knowledge Manager</p>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    for num, name, desc in steps:
        st.markdown(f"""
        <div style='background:#0f1219;border:1px solid #1e2333;border-radius:8px;padding:10px 16px;margin:4px 0;font-size:13px'>
...
with col_left:
    ...
    uploaded = st.file_uploader("Drop your tickets CSV here", type=["csv"])

    if uploaded:
        df = pd.read_csv(uploaded)
        st.metric("Tickets loaded", len(df))
        with open("tickets_upload.csv", "wb") as f:
            f.write(uploaded.getbuffer())
        with st.expander("Preview data"):
            st.dataframe(df[["ticket_id","title","category","subcategory","status"]].head(8), use_container_width=True)

    st.markdown("#### Or use sample data")
    use_sample = st.button("▶  Run with tickets.csv", use_container_width=True)
    st.markdown("<br>", unsafe_allow_html=True)

    can_run = uploaded is not None or use_sample
    run_button = st.button("🚀  Generate KB Articles", use_container_width=True, disabled=not can_run)
...
with col_right:
    ...
    if run_button or use_sample:
        ...
        def log(msg):
            logs.append(msg)
            log_area.markdown(
                "<div style='background:#0f1219;border:1px solid #1e2333;border-radius:8px;padding:12px 16px;font-size:12px;color:#475569;max-height:180px;overflow-y:auto'>" +
                "<br>".join(logs[-12:]) + "</div>",
                unsafe_allow_html=True
            )
        ...
        try:
            ...
            total_tix = sum(r['ticket_count'] for r in results)
            m1.metric("Clusters",       len(results))
            m2.metric("Avg Confidence", f"{avg_conf}/100")
            m3.metric("Avg Deflection", f"~{avg_defl}%")
            m4.metric("Tickets Used",   total_tix)

            st.markdown("---")

            for r in results:
                color = "#22c55e" if r['confidence_score'] >= 80 else "#f59e0b" if r['confidence_score'] >= 60 else "#ef4444"
                st.markdown(f"""
                <div class='cluster-card' style='border-left-color:{color}'>
                  <div style='display:flex;justify-content:space-between;align-items:center'>
                    <div>
                      <span style='font-size:10px;letter-spacing:3px;text-transform:uppercase;color:#475569'>{r['cluster_id']}</span>
                      <h3 style='font-family:Syne,sans-serif;font-size:18px;color:#f0f4ff;margin:4px 0'>{r['topic']}</h3>
                    </div>
                    <div style='text-align:right'>
                      <div style='font-size:24px;font-weight:800;color:{color}'>{r['confidence_score']}/100</div>
            ...
            st.markdown("""
            <div style='margin-top:24px;padding:16px 20px;background:#0a1a0a;border:1px solid #14532d;border-radius:8px;font-size:13px'>
        ...
        <div style='text-align:center;padding:60px 20px;color:#1e2333'>
          ...
          <div style='font-family:Syne,sans-serif;font-size:18px;color:#1e2a40;margin-bottom:8px'>Upload a CSV or click Run with sample data</div>
          <div style='font-size:12px;color:#1a2030'>Pipeline will cluster tickets, draft KB articles,<br>review them and email to Knowledge Manager</div>
        </div>
        """, unsafe_allow_html=True)